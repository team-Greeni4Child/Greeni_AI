...
def _system_base() -> str:
    ...
    return (
        ...
        "항상 존댓말을 사용하고, 이모티콘/과장된 감탄을 금지합니다. "
...
def _role_instruction(role: RoleType) -> str:
    if role == "shop":
        return (
            "역할: 상점 주인과 손님. "
            "목표: 물건 설명, 가격/교환/환불 규칙 안내, 예의 바른 접객. "
            "필요시 1~2개의 선택지로 아이가 쉽게 고를 수 있게 도와주세요."
        )
    if role == "teacher":
        return (
            "역할: 선생님과 학생. "
            "목표: 개념을 쉽게 설명하고, 이해를 확인하는 질문 1개를 마지막에 제시. "
...
def reply(req: RoleplayRequest, sid: Optional[str] = None) -> dict:
    ...
    model = req.model or getattr(settings, "CHAT_MODEL", "gpt-4o-mini")
